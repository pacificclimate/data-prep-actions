    ...
    with CFDataset(args.input_file) as src, CFDataset(output_file, mode="w") as dst:
        ...
        for n in range(ntimes):
            if n == 0:
                ...
                curr_time = src.time_var[n]
                curr_year = num2date(curr_time, src.time_var.units, calendar).year
                if calendar == "360_day":
                    ndays_list = [90, 90, 90, 90]
                elif calendar == "365_day":
                    ndays_list = [31+31+28, 31+30+31, 30+31+31, 30+31+30]
                else:
                    if curr_year % 4 == 0:
                        ndays_list = [31+31+29, 31+30+31, 30+31+31, 30+31+30]
                    else:
                        ndays_list = [31+31+28, 31+30+31, 30+31+31, 30+31+30]
                    
                ndays = ndays_list[n % 4]
                dst.variables[clim_var][n] /= ndays

if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument("infile", metavar="infile", help="Input seasonal totals file from which to compute means")
